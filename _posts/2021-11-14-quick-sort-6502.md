---
layout: post
title: 📊 Quick Sort with 6502 Assembly
...
This also uses [Minicube64][] to run, so a lot of this is necessary for that architecture. If you want the meat and potatoes, check out the `ENUM`, the variable initialization, and the `QuickSort` label and see how it all works.

[Quick sort](https://en.wikipedia.org/wiki/Quicksort) generally uses recursion, which is fairly difficult to do in a way that feels traditional, but it *does* end up being depth-first. I ended up using the stack to handle the high and low boundaries of the subarrays, and it works on all arrays I tried it on 🤷‍♀️.
